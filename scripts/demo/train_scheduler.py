#!/usr/bin/env python3
"""M4 viability demo: train a LEARNED scheduler on randomized multi-site construction layouts and
compare it to the greedy nearest-batch planner and to random.
...
sites served by one rover/one drum the leg ORDER controls makespan. Action = one trip-leg
(Discrete over regions: load the drum at a borrow pit, or dump it toward a build site via fill_toward).

Observed on a 200k-step run (held-out randomized layouts, seed 0):
    greedy  success=100%  avg_legs=28
    random  success~=3%
    PPO     success=100%  avg_legs=27   (matches/edges the planner, generalizes across layouts)
